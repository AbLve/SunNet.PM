...
{
    public partial class WaitingResponse : TicketPageHelper
    {
        ...
        int page = 1;
        int recordCount;
        private UsersEntity userEntity { get; set; }
        ...
        {
            List<TicketsState> ticketStatus = (userEntity.Role == RolesEnum.PM || userEntity.Role == RolesEnum.Sales) ? TicketsStateHelper.SunnetUSAllowShowStatus : TicketsStateHelper.SunnetSHAllowShowStatus;
        ...
        private string GetSelectedTicketStatus()
        {
            ...
            var cache = new CookieHelper().Get($"TicketStatus_{userEntity.Role}_{DateTime.Now.ToString("yyyyMMdd")}");
        ...
        private void TicketsDataBind()
        {
            ...
            List<TicketsEntity> list = ticketAPP.SearchTicketsForAllTickets(userEntity, txtKeyWord.Text, project, statusStr, QS("create"), type, priorith, assignuser,
                 CurrentPageIndex, anpOngoing.PageSize, OrderBy, OrderDirection, true, out recordCount);
        ...
        private bool CheckSecurity(int pid)
        {
            ...
            list = proApp.GetUserProjects(userEntity);
        ...
        protected string TicketCreateUser(string firstName, string lastName, bool isEstimates)
        {
            if (isEstimates)
            {
                if (userEntity.Office == "US" && (userEntity.Role == RolesEnum.PM || userEntity.Role == RolesEnum.Sales || userEntity.Role == RolesEnum.ADMIN))
