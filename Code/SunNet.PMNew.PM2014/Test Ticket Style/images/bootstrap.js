...
+function ($) {
    ...
    Modal.prototype.reload = function () {
        ...
        if (this.options.remote) {
            var url = this.options.remote.indexOf("?") > 0 ? this.options.remote : this.options.remote + "?"
            url += "&parentmodal=" + this.$element.attr("id")
            url += "&_=" + Math.random()
            this.$element
    ...
    Modal.prototype.removeLoading = function () {
        clearTimeout(this.loadingEvent)
        this.$element.find(".loading").hide()
    }

    Modal.prototype.loading = function () {
        var self = this
        this.loadingEvent = setTimeout(function () {
    ...
    $.fn.modal = function (option, _relatedTarget) {
        return this.each(function () {
            ...
            if (data && data.options.remote !== $(_relatedTarget).attr("href")) {
                data.options.remote = $(_relatedTarget).attr("href")
                data.$element.find("iframe").attr("src", data.options.remote)
            }
    ...
    // MODAL default html
    // ==================
    function GetModal(width, id) {
        var $modal = $('<div class="modal " id=' + id.replace("#", "") + '><div class="modal-dialog"><div class="modal-content"><div class="modal-body"><iframe></iframe><div class="loading"></div></div></div></div></div>')
        $modal.find(".modal-content").width(width)
        $modal.find("iframe").attr("scrolling", "no").attr("frameborder", 0)
            .css({ overflow: "hidden", border: "0px", width: "100%", height: "100%" })
    ...
    $(document).on('click.bs.modal.data-api', '[data-toggle="modal"]', function (e) {
        ...
        if ($target.length === 0) $target = GetModal(500, $this.attr('data-target'))
