...
namespace SunNet.PMNew.WinLogin
{
    partial class BrowserControl : UserControl
    {
        ...
        void _browser_StartNewWindow(object sender, BrowserExtendedNavigatingEventArgs e)
        {
            ...
            frmTestBrowser mf = GetfrmTestBrowserFromControl(sender as Control);
            if (mf == null)
                return;
        ...
        // Used for obtaining the frmTestBrowser from a control
        private static frmTestBrowser GetfrmTestBrowserFromControl(Control control)
        {
            while (control != null)
            {
                if (control is frmTestBrowser)
                    break;
                control = control.Parent;
            }
            return control as frmTestBrowser;
        }
