...
(function ($) {
    ...
    $.fn.photoTag = function (options) {
        ...
        var wrapImage = function (image, image_id) {
            ...
            var canvas = $('<div id="' + options.imageWrapBox.canvasIdPrefix + image_id + '" style="position:relative;height:564px;width:718px;"></div>');
            ...
            if (!$.isFunction(options.beforeTagRequest) || options.beforeTagRequest(parameters)) {
                $.getJSON(
					...
					function (response) {
					    ...
					    if (response.options) {
					        var tag = $.extend({}, options.tag, response.options.tag);
					        response.options.tag = tag;
