...
var loadedExeced = false;
var loadedEvent = 0;

// Mark Coming soon...
function _doload_ComingSoon() {
    if (_top.svgweb && !loadedExeced) {
        var tree = document.getElementById("myTree");
        //var text = CreateObject(120, 120, 0, 0, {fill: "#ff0000", style:"font-size:50px;"}, "text");  
        //tree.appendChild(text);
        var image = CreateObject(340, 130, 445, 135, {}, "image", "/Images/icons/comingsoon.png");
        tree.appendChild(image);

        loadedExeced = true;
        clearTimeout(loadedEvent);
    }
    else {
        clearTimeout(loadedEvent);
        loadedEvent = setTimeout(function () { _doload(); }, 100);
    }
}
function CreateObject(x, y, width, height, others, type, src) {
    var obj = null;
    if (type == "text") {
        var text = document.createTextNode("Coming soon...", true);
        obj = document.createElementNS(SVG, type);
...
function doload() {
    if (_top.svgweb && !loadedExeced) {
        ...
        clearTimeout(loadedEvent);
        loadedEvent = setTimeout(function () { doload(); }, 100);
    }
}
...
function FamilyGraph(options) {
    ...
    this.getPrevPeople = function (id) {
        if (!this.peopleTree || !this.peopleTree.length) { return {}; }
        var result = {};
        for (var i = 0; i < this.peopleTree.length; i++) {
            for (var j = 0; j < this.peopleTree[i].length; j++) {
                if (j > 0 && this.peopleTree[i][j].ID == id) {
                    result = this.peopleTree[i][j - 1];
                }
            }
        }
        return result;
    ...
    this.PrapareData = function () {
        ...
        for (var j = 0; j < firstLevel.length; j++) {
            if (this.peoples[j].Gender == this.settings.genderMap.Male || !this.peoples[j].Husband) {
                ...
                // 根据 flag 调整祖先顺序
                if (this.getFlag().ISMyGrandF(people)) {
                ...
                if (people.Wife) {
                    this.peopleTree[0].push(people.Wife);
                }
            }
        }
        // 其他级别数据源
        for (var i = 1; i < this.Levels; i++) {
            ...
            for (var j = 0; j < this.peopleTree[i - 1].length; j++) {
                ...
                for (var k = 0; k < _childRen.length; k++) {
                    ...
                    if (_top.FG.inArray(people, this.peopleTree[i]) < 0) {
            ...
            if (levelPeople && levelPeople.length) {
                for (var m = 0; m < levelPeople.length; m++) {
                    if (_top.FG.inArray(levelPeople[m], this.peopleTree[i]) < 0) {
    ...
    this.drawPeople = function (people) {
        ...
        if (people.Status == this.settings.statusMap.Normal) {
            // action: add Member
            if (typeof people.buttonCount == "undefined") { people.buttonCount = 0 };
            var x = people.x + this.settings.objects.Button.Left + (this.settings.objects.Button.Width + this.settings.objects.Button.Space) * people.buttonCount;

            var action1 = this.createImage("/images/icons/treeadd.png", x, people.y + this.settings.objects.Button.Top,
            this.settings.objects.Button.Width, this.settings.objects.Button.Height, {
            ...
            var action1Tooltip_Bg = this.createTooltipBG("people" + people.ID + "ActionAdd", x + this.settings.objects.Tooltip.Left,
                people.y + this.settings.objects.Button.Top + this.settings.objects.Tooltip.Top,
            ...
            var action1Tooltip_Text = this.createTooltipText("people" + people.ID + "ActionAdd", "Add family member", x + this.settings.objects.Tooltip.Left + 1,
                people.y + this.settings.objects.Button.Top + this.settings.objects.Tooltip.Top + 15 + 1,
        ...
        // action edit
        if (typeof people.buttonCount == "undefined") { people.buttonCount = 0 };
        x = people.x + this.settings.objects.Button.Left + (this.settings.objects.Button.Width + this.settings.objects.Button.Space) * people.buttonCount;
        var action2 = this.createImage("/images/icons/treeedit.png",
        x,
        people.y + this.settings.objects.Button.Top,
        this.settings.objects.Button.Width, this.settings.objects.Button.Height, {
        ...
        if (people.Status == this.settings.statusMap.Normal && people.ID != this.flagID) {
            if (typeof people.buttonCount == "undefined") { people.buttonCount = 0 };
            x = people.x + this.settings.objects.Button.Left + (this.settings.objects.Button.Width + this.settings.objects.Button.Space) * people.buttonCount;
    ...
    this.createPeopleRect = function (people, prevPeople) {
        ...
        var _wife;
        var _husband;
        var _prevForNext = people;
        if (!_people.drawed) {
            if (prevPeople.ID) {
                prevPeople = prevPeople.getMaxX();
            }
            ...
            // 根据父母节点决定自己位置
            var _leftP = _people.getLeftParent();
            var _parent = _people.getRightParent();
            var parentPrev = _parent && this.getPrevPeople(_parent.ID);

            if ((_leftP || _parent)) {
                var center = 0;
                if (!parentPrev || !_people.Father || !_people.Mother) center = _parent.x + _parent.width / 2;
                else if (_leftP == parentPrev) center = _leftP.x + _leftP.width + _leftP.intervalX / 2;
                else center = parentPrev.x + parentPrev.width + parentPrev.intervalX / 2;

                var minLeft = 0;
                var mcc = _parent.MaxChildrenCount;
                if (mcc < 0 && _parent.Children && _parent.Children.length) mcc = _parent.Children.length;
                if (mcc < 0 && _leftP && _leftP.MaxChildrenCount > 0) mcc = _leftP.MaxChildrenCount;
                if (mcc < 0) mcc = 0;
                var maxWidth = mcc * (this.settings.objects.People.Width + this.settings.objects.People.intervalX) - this.settings.objects.People.intervalX;

                minLeft = center - maxWidth / 2;
                if (mcc < 2) {
                    minLeft = center - this.settings.objects.People.Width / 2;
                }
                if (parentPrev && _leftP != parentPrev) {
                    var compareLeft = parentPrev.x + parentPrev.Width + parentPrev.intervalX / 2;
                    if (minLeft < compareLeft)
                        minLeft = compareLeft;
                }

                // 配置节点偏移，迎合父级(可能是父亲或母亲)节点位置
                if (typeof _parent.drawedChildren == "undefined") _parent.drawedChildren = 0;
                var childPosition = _parent.drawedChildren;
                var shouldBex = minLeft + childPosition * (this.settings.objects.People.Width + this.settings.objects.People.intervalX);
                if (_people.x < shouldBex) {
                    _people.x = shouldBex;
                }
                _parent.drawedChildren++;
            }

            if (_people.MaxChildrenCount > 0 && _people.Children && _people.Children.length) {
                // 配置父节点偏移，孩子留出位置
                var mcc = _people.MaxChildrenCount;
                if (_people.Wife && _people.Wife.Children) mcc = Math.min(_people.MaxChildrenCount, _people.Wife.Children.length);
                if (_people.Husband && _people.Husband.Children) mcc = Math.min(_people.MaxChildrenCount, _people.Husband.Children.length);
                var maxWidth = mcc * (this.settings.objects.People.Width + this.settings.objects.People.intervalX) - this.settings.objects.People.intervalX;
                var adjust = 0;
                if (_wife || _husband)
                    adjust = (maxWidth - _people.intervalX - this.settings.objects.People.Width * 2) / 2;
                else
                    adjust = (maxWidth - this.settings.objects.People.Width) / 2;
                if (mcc < 2) adjust = 0;
                _people.x += adjust
                if (_wife) {
            ...
            _people.drawed = true;
            _prevForNext = _people;

            if (_wife && !_wife.drawed) {
    ...
    this.Draw = function (peoples) {
        ...
        for (var i = 0; i < peoples.length; i++) {
            var _tp = peoples[i].slice(0);
            if (i > 0)
                peoples[i] = _top.FG.quickSortByParent(_tp);
            var _prevPeople = _top.jQuery.extend({}, this.settings.defaultPrev,
            ...
            for (var j in peoples[i]) {
                ...
                _prevPeople = this.createPeopleRect(_people, _prevPeople);
            }
        }
    }
    ...
    this.DrawLine = function () {
        ...
        for (var i = 0; i < this.relations.length; i++) {
            ...
            if (relationType == this.settings.relationMap.Husband) {
                if (typeof source.drawedEx == "undefined") source.drawedEx = 0;
                if (relation.Status == "Expired") {
                    target.exIndex = source.drawedEx;
                    source.drawedEx++;
                }
                var path = [];
                path.push({ x: source.x + source.width, y: source.y + source.height / 2 + target.getExHeight() });
                path.push({ code: this.settings.pathCode.lineto, x: target.x, y: target.y + target.height / 2 + target.getExHeight() });
                this.drawRelation(path, source, target);
            }
            else if (relationType == this.settings.relationMap.Father) {
                var path = [];
                var father;
                var mother;
                ...
                if (!father || !mother) {
                    ...
                    var rightParentPrev = this.getPrevPeople(rightParent.ID);
                    if (rightParentPrev) {
                        var relationX = rightParentPrev.x + rightParentPrev.width + (rightParent.x - rightParentPrev.x - rightParentPrev.width) / 2;
                        path.push({
                            x: relationX,
                            y: rightParent.y + rightParent.height / 2 + rightParent.getExHeight()
                        });
                        path.push({
                            code: this.settings.pathCode.lineto,
                            x: relationX,
                            y: rightParent.y + rightParent.height + rightParent.intervalY / 2
                        });
                        path.push({ code: this.settings.pathCode.lineto, x: target.x + target.width / 2, y: source.y + source.height + source.intervalY / 2 });
                        path.push({ code: this.settings.pathCode.lineto, x: target.x + target.width / 2, y: target.y });
                    }
                    ...
                    this.drawRelation(path, rightParent, target);
                }
                else {
    ...
    this.startDraw = function () {
        ...
        this.tooltipGroup = this.createSVGGroupIFNo("tooltips", false);
        ...
        this.navSettings = new this.settings.navigation(self);
        ...
        _top.FG.each(this.peoples, function (index, people) {
            ...
            if (people.y + people.height > self.navSettings.tree.Real.height) {
                self.navSettings.tree.Real.height = people.y + people.height + 100;
            }
        });
        ...
        if (this.navSettings.tree.Real.width > this.navSettings.tree.Show.width || this.navSettings.tree.Real.height > this.navSettings.tree.Show.height) {
            ...
            if (this.getMySelf() && this.getMySelf().x > this.navSettings.tree.Show.width) {
                var _left = 0;
