...
var FG = {
    ...
    createPerson: function (people) {
        var p = new Person();
        this.extend(p, people, { Level: 0, MaxChildrenCount: 0 });
...
function Person(datas) {
    ...
    this.IsOK = datas && datas["IsOK"];

    this.VirtualAccount = datas && datas["VirtualAccount"];
    this.Level = 0;
        ...
        if (this.Wife && this.Wife.Children && this.Wife.Children.length) {
            for (var i = 0; i < this.Wife.Children.length; i++) {
                if (cr.indexOf(this.Wife.Children[i]) < 0) {
                    //this.Wife.Children[i].Father = this;
