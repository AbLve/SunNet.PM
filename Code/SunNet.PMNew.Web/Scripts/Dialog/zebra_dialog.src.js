    ...
    $.Zebra_Dialog = function () {
        ...
        plugin.init = function () {
            ...
            if (plugin.settings.title) {
                ...
                jQuery('<div>', {
                    ...
                    'class': 'ZebraDialog_Title_Content',

                    'style': 'width: ' + (plugin.dialog.width() - button_Width) + "px"

                }).html(plugin.settings.title).appendTo(title_container);

                var ZebraDialog_Title_Button = jQuery('<div>', {

                    'class': 'ZebraDialog_Title_Button',

                    'style': 'width: ' + (button_Width - 44) + "px"

                }).appendTo(title_container);

                var ZebraDialog_Title_Button_A = jQuery("<a>");

                ZebraDialog_Title_Button_A.bind('click', function (e) {
                ...
                ZebraDialog_Title_Button.append(ZebraDialog_Title_Button_A);
                title_container.appendTo(plugin.dialog);
            }
            ...
            //@height
            var tmpHeight = parseInt(plugin.settings.height);
                    ...
                    switch (type) {
                        ...
                        case 'ajax':
                            ...
                            ajax_options.success = function (result) {
                                ...
                                var $forms = canvas.find("form:not([nowarn])[id^='F_']");
                                setTimeout(function () {
                                    $forms.each(function (index, form) {
                                        var $this = jQuery(form);
                                        $this.data("SourceData", $this.serialize());
                                    });
                                }, 1000);
                                /*Custom Title*/
                                canvas.find("._title").each(function () {
                                    jQuery(this).tooltip();
                                });
        ...
        */
        plugin.close = function (caption) {
            var $lastDialog = $(document).find(".ZebraDialog:last");
            var $form = $lastDialog.find("form[id^='F_']:first");
            var formID = $lastDialog.find("form[id^='F_']").attr("id");
            var changed = $form.length && $form.MdiForm("Changed");
            if (changed) {
                if (!confirm(top.message.warnBeforeFormLeave)) {
                    return false;
                }
            }
            ...
            // when the animation is complete
                function () {
                    ...
                    $(document).find(".ZebraDialogOverlay:last").remove();
                });
            ...
            function () {
                ...
                if (plugin.settings.onClose && typeof plugin.settings.onClose == 'function')
                    ...
                    plugin.settings.onClose(undefined != caption ? caption : '', formID);
...
$.Zebra_Dialog.closeCurrent = function (elem) {
    $(elem).closest(".ZebraDialog").find(".ZebraDialog_Title_Button a").click();
}

$.Zebra_Dialog.popWindow = function (_url, _title, _width, _height, _callback) {
    var _source = { 'ajax': _url };
    new $.Zebra_Dialog("", {
        keyboard: false,
        max_height: 600,
        'source': _source,
...
function popConfirm(title) {
    $.Zebra_Dialog({ "title": title, "type": "information", 'modal': true, "buttons": true });
}
