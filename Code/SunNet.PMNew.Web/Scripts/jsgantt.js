...
JSGantt.TaskItem = function(pID, pName, pStart, pEnd, pColor, pLink, pMile, pRes, pComp, pGroup, pParent, pOpen, pDepend, pCaption)
{
    ...
    JSGantt.colorSet=[
  {color:'cc6666',hasSelected:false}
  ,{color:'9966ff',hasSelected:false}
  ,{color:'996633',hasSelected:false}
  ,{color:'9933cc',hasSelected:false}
  ,{color:'9999cc',hasSelected:false}
  ,{color:'99cc99',hasSelected:false}
  ,{color:'999900',hasSelected:false}
  ,{color:'66ffff',hasSelected:false}
  ,{color:'66ff33',hasSelected:false}
   ,{color:'6633ff',hasSelected:false}
  ,{color:'663300',hasSelected:false}
  ,{color:'666699',hasSelected:false}
  ,{color:'669900',hasSelected:false}
   ,{color:'66cc33',hasSelected:false}
  ,{color:'00ccff',hasSelected:false}
  ,{color:'000099',hasSelected:false}
  ,{color:'009900',hasSelected:false}
  ,{color:'FFFF33',hasSelected:false}
  ,{color:'FFCC99',hasSelected:false}];
  JSGantt.getColor=function()
  {
  var coloSet=JSGantt.colorSet;
  var colorSetLength=coloSet.length;
     for (var i=colorSetLength ;i--;)
     {
        if(coloSet[i].hasSelected==false)
        {
        coloSet[i].hasSelected=true;
        return coloSet[i].color;
        }
     }
  }

  JSGantt.ScheduleItem=function(scheduleID, scheduleName, startDate, endDate)
  {
    return new JSGantt.TaskItem(scheduleID,scheduleName,startDate,endDate,JSGantt.getColor(),"",0,'',0,0,-1,0);
  }
   ...
   this.Draw = function()
   {
      ...
      if(vTaskList.length > 0)
      {
         ...
         vLeftTable += '</TR>';
            ...
            if(!vShowLeft)
            {
            overFlowXstyle='style="overflow-x:hidden"';
            }
            vRightTable = 
            '<TD style="width: ' + vChartWidth + 'px;" vAlign=top bgColor=#ffffff>' +
            '<DIV class=scroll2 '+overFlowXstyle+' id=rightside>' +
         ...
         else
         {
           if(vFormat == 'hour')
            {
                vRightTable += '<td class=gdatehead style="FONT-SIZE: 12px; HEIGHT: 19px;" align=center colspan=24>' ;
                vRightTable += JSGantt.formatDateStr(vTmpDate, "mm/dd/yyyy") + '</td>';
                vTmpDate.setDate(vTmpDate.getDate()+1);
            }
         }
         ...
         var vTmpHourMaxDate=new Date();
         vTmpHourMaxDate.setFullYear(vMinDate.getFullYear(), vMinDate.getMonth(), vMinDate.getDate(),0,0);
         vTmpHourMaxDate.setHours(23);
         vTmpHourMaxDate.setMinutes(0);
         vTmpHourMaxDate.setMilliseconds(0);
            maxDate=vTmpHourMaxDate;
         }
         else
         {
            maxDate=vMaxDate;
         ...
         {	
            ...
            else if (vFormat == 'hour')
            {
			   ...
			   if(  vTmpDate.getHours() ==0  ) 
                ...
                if(!vShowLeft&& Date.parse(vTmpDate) == Date.parse(maxDate))
                {
                vDateRowStr += '<td class="ghead" style="BORDER-TOP: #efefef 1px solid; FONT-SIZE: 12px; HEIGHT: 19px; BORDER-LEFT: #efefef 1px solid; border-right:1px solid #efefef"  bgcolor=#' + vWeekdayColor + ' align=center><div style="width: '+vColWidth+'px">' + vTmpDate.getHours() + '</div></td>';
                 vItemRowStr += '<td class="ghead" style="BORDER-TOP: #efefef 1px solid; FONT-SIZE: 12px; BORDER-LEFT: #efefef 1px solid; cursor: default; border-right:1px solid #efefef; "  bgcolor=#' + vWeekdayColor + ' align=center><div style="width: '+vColWidth+'px">&nbsp&nbsp</div></td>';
                }
                else
                {
         ...
         for(i = 0; i < vTaskList.length; i++)
         {
	       ...
	       else if (vFormat=='minute')
	       {
            ...
            if( vTaskList[i].getMile()) {
               ...
               // Build date string for Title
               vDateRowStr = JSGantt.formatDateStr(vTaskStart,'hh:mm') + ' - ' + JSGantt.formatDateStr(vTaskEnd,'hh:mm')
                ...
                {
                var dateDifference=Date.parse(vTaskList[i].getEnd()) - Date.parse(vTaskList[i].getStart());
                if(!vShowLeft)
                {
                 vTaskRight = dateDifference / ( 60 * 60 * 1000);
                }else
                {
                    vTaskRight = dateDifference / ( 60 * 60 * 1000) + 1/vColUnit;
                    }
...
JSGantt.formatDateStr = function(pDate,pFormatStr) {
     ...
     var  vHourStr= pDate.getHours()+' ';
     var  vMinutes=pDate.getMinutes()==0? ' 00':' '+pDate.getMinutes();
      var vDateStr = "";	
