            ...
            onAfterEdit: function(index, row) {
                ...
                if (url) {
                    if (opts.onBeforeSubmit) {
                        if (opts.onBeforeSubmit.call(target, index, row)) {
            ...
            $.messager.confirm(opts.destroyMsg.confirm.title, opts.destroyMsg.confirm.msg, function(r) {
                if (r) {
                    ...
                    if (row.isNewRecord) {
                        ...
                        if (opts.destroyUrl) {
                            var idValue = row[opts.idField || 'id'];
                            $.post(opts.destroyUrl, { id: idValue }, function(responseData) {
                                if (opts.tree) {
                                    dg.datagrid('reload');
                                    var t = $(opts.tree);
                                    var node = t.tree('find', idValue);
                                    ...
                                    dg.datagrid('cancelEdit', index);
                                    // delete success remove row from table 
                                ...
                                opts.onDestroySuccess.call(dg[0], index, row, responseData);
                            }, 'json');
                        } else {
                            dg.datagrid('cancelEdit', index);
                            dg.datagrid('deleteRow', index);
