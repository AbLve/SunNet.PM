...
{
    public partial class ListTicketDrafted : BaseWebsitePage
    {
        #region declare
        ...
        private void TicketsDataBind(string projectID)
        {
            ...
            dto.Status = ((int)TicketsState.Draft).ToString();
            dto.TicketType = this.ddlTicketType.SelectedValue;
            ...
            int pageCount = ticketAPP.GetDraftedTicketsCount(UserInfo.ID, int.Parse(dto.Project), int.Parse(dto.Company), ticketType, dto.KeyWord);

            if (pageCount > 0)
            {
                list = ticketAPP.GetDraftedTicketsList(UserInfo.ID, int.Parse(dto.Project), int.Parse(dto.Company), ticketType
        ...
        private void TicketSatusBind()
        {
            ...
            int[] array = { (int)TicketsState.Draft };
        ...
        public string ShowDeleteImg(string tid)
        {
            StringBuilder sb = new StringBuilder();

            return sb.AppendFormat("<a href='###' id={0} onclick='DeleteConfirm({0})'><img alt='delete' title='Delete' src='/icons/26.gif'></a>", tid).ToString();
        }
